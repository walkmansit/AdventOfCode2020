...
object Day4 {
    ...
    fun getResult() : Int {
        return input.split("\n\n").asSequence().map { Passport.fromInput(it) }.count { it.isValidKeysOnly() }
    }

    fun getResultAdvanced() : Int {
        return input.split("\n\n").asSequence().map { Passport.fromInput(it) }.count { it.isValid() }
    }
    ...
    data class Passport private constructor(val data:Map<String,String>) {
        ...
        private fun validateStringToNumbersByLenghtAndDiapason(value : String, lenght : Int, from: Int, to:Int) : Boolean{
