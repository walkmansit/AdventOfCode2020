...
object Day7 {
    ...
    private class Graph {
        ...
        fun countParentsForNode(name : String) : Int {
            fun getUniqueParents(node : Node) : MutableSet<String> {
                val result = mutableSetOf<String>()

                for ((k,v) in node.parents) {
                    result.add(k)
                    result.addAll(getUniqueParents(v))
                }
                return result
            }
            ...
            if (nodes.containsKey(name)){
                ...
                return getUniqueParents(targetNode).size
        ...
        companion object {
            ...
            fun fromInput(inp : String) : Graph {
                val lines = inp.split(".\n")

                val gph = Graph()

                for(i in 1..lines.size-2) {
                    val lineParts = lines[i].split(" bags contain ")
